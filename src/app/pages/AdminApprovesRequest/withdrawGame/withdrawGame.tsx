...
import { useFilterWithdrawGameRequest } from '../components/FilterContext/FilterWithdrawGameProvider';
import { CartRequestWithdrawGame } from '../components/CartRequestWithdrawGame';

interface InPropsStyle {}
export const WithdrawGame = () => {
  ...
  const requests = useSelector(selectWithdrawGame);
  ...
  const calledWithdrawGame = useSelector(selectCalledWithdraw);

  useLayoutEffect(() => {
    return () => {
      dispatch(requestActions.resetDataWithdrawGame());
  ...
  useLayoutEffect(() => {
    if (calledWithdrawGame) {
      setLoading(false);
    }
  }, [calledWithdrawGame]);

  //  "submenu-withdrawGame-request": "Withdrawal request to game",
  // "Withdrawal Request to Game": "Withdrawal Request to Game",
  ...
  return (
    <>
      ...
      <NavContainer
        laberHeader={t('approveWithdraw.Withdrawal Request to Game')}
        propsFilter={
