...
import { useFilterPlusRequest } from '../components/FilterContext/FilterPlusProvider';
interface InPropsStyle {
  mobile?: boolean;
}
export const RPlus = () => {
  const dispatch = useDispatch();
  const data = useSelector(selectDataAddition);
  ...
  return (
    <NavContainer
      laberHeader={t('Header.submenu-request-plus')}
      propsFilter={
        ...
        {responseRequest.error === 0 &&
          (data?.length == 0 ? (
            <Center>
              <Text>
                {isFilter
                  ? t('approveWithdraw.No results required to add appropriate money !')
                  : t('approveWithdraw.There is no request for money !')}
              </Text>
            </Center>
          ) : (
            <InfiniteScroll
              ...
              endMessage={
                <Center className={c.centerOver}>
                  <Text>{t('approveWithdraw.The request is over !')}</Text>
                </Center>
              }
            >
