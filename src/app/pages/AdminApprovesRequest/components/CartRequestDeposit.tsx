...
export const CardRequestDeposit = (props: IpropsStyle) => {
  ...
  return (
    <Flex className={c.boxDepsit}>
      ...
      <Flex
        onClick={() => {
          props.service === 1
            ? navigate(`${checkUrl ? '/request-plus' : '/approves-recharge'}/detail/${props.id}`)
            : navigate(`${checkUrl ? '/request-deduction' : '/approves-withdraw'}/detail/${props.id}`);
      ...
      >
        <Flex className={c.infoDeposi}>
          ...
          <Flex mr={10}>
            <Text fw={700} fz={16} className="text2 lh">
              {`${props?.status === '1' ? props?.realExchange : props?.money}`}
            </Text>
            <CoinUsdt />
          </Flex>
        </Flex>
        ...
        <Flex className="code">
          <Text fw={700} ml={10} className="lh">
            {t('UserManagement.history.phoneNumberOfUser')}
          </Text>
          <Text fw={700} mr={10} className="lh">
            {clearPhoneCode84(props?.phoneCodeReceiver ?? props?.phoneCode)}
          </Text>
        </Flex>
        <Flex className="time">
