...
const FilesUploadBanner = memo(
  ({ label, files, form, setFiles }: FilesUploadBannerProps) => {
    const { classes } = makeStyles();

    function handleChange(data: File[]) {
      // Create BLOB file
      if (data.length + files.length > 5) {
        form?.setFieldError('cover_photo', 'Không được upload quá 4 ảnh');
      ...
      Array.from(data).forEach(async (file, index) => {
        try {
          ...
          if (response.data.status === 200) {
            const findData = blobFiles.find(
              (item, _) => item.name === file.name,
            );
        ...
        <Flex className={classes.flex}>
          {files?.map((item, index) => (
            <Box className={classes.box} key={index}>
              ...
              {!item?.done && (
                <Progress value={item?.progress} color="var(--primary-1)" />
              )}
            </Box>
          ))}
          {files.length < 5 && (
